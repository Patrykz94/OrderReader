...
public interface IParseOrder
{
    ...
    CustomerProfile? GetCustomerProfile(Dictionary<string, string[]> orderText, CustomersHandler customers);
    ...
    /// <param name="customerProfile">A <see cref="CustomerProfile"/> object that holds all information about this customer, products, and depots</param>
    /// <param name="ordersLibrary">An <see cref="OrdersLibrary"/> object that holds all processed orders</param>
    Task ParseOrderAsync(Dictionary<string, string[]> orderText, string fileName, CustomerProfile customerProfile, OrdersLibrary ordersLibrary);
}